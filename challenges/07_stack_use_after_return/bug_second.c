...
{
  // 힙 방식으로 바꾸기 (malloc()힙에 MAX_LINES개의 char * 공간 확보. 그 공간의 시작 주소를 Parts에 저장)
  // MAX_LInes가 8개이고 8바이트랑 곱하면 64바이트를 확보함.
  // char * 포인터들을 저장할 공간임.
...
int main(void)
{
  ...
  for (int i = 0; i < v.count; i++)
    // v.lines를 더이상 사용하지 않으면 free()
    // unsigned char 문자를 부호 없는 1바이트 정수로 해석하라는 형변환임.
    // 0~255 범위의 숫자로 해석함.
