...
static void *arena_alloc(size_t n, unsigned char *arena, size_t *arena_off)
{
    // 현재까지 사용한 크기 (*arena_off)에 새로 필요한 크기(n)을 더했을때
    // ARENA_SIZE를 초과하면 arena범위를 벗어나므로 할당하지 않음.
    // sizeof(arena)라고 하지 않는 이유는 여기에서 arena가 포인터임. sizeof(arena)는 포인터 크기가 됨.
    if (*arena_off + n > ARENA_SIZE)
        return NULL;
    void *p = &arena[*arena_off];
    // n바이트를 사용했으므로 다음 할당위치를 N만큼 뒤로 이동함.
    *arena_off += n;
    return p;
}

// intern()에서 이미 size_t *arena_off라고 선언했기 때문에
// arena_off 자체는 offset을 나타내는 숫자이고, 원본 값을 변경해야하므로 &arena_off로 주소를 전달함.
// offset은 기존 위치에서 얼마나 떨어져있는지를 나타내는 숫자임.
static char *intern(const char *s, unsigned char *arena, size_t *arena_off)
{
    size_t n = strlen(s) + 1;
    char *dst = arena_alloc(n, arena, arena_off);
    if (dst != NULL)
...
int main(void)
{
    unsigned char arena[ARENA_SIZE]; /* main이 실행되는 동안 유지된느 지역 아레나 */
    size_t arena_off = 0;
    ...
    for (int i = 0; i < 100000; i++)
    {
        ...
        // 배열 arena를 함수에 넘기면 배열의 첫번째 원소 주소가 전달됨.
        // 그래서 받는 쪽은 char *arena 포인터가 되는 것임.
        // arena_off 자체가 숫자이므로, 원본을 변경하려면 주소를 넘겨야 함.
        // 그래서 받는 쪽은 size *arena_off가 되는 것임.

        last = intern(buf, arena, &arena_off);
        // 여기도 추가해야지
        if (last == NULL)
        {
            fprintf(stderr, "arena_full\n");
            return 1;
        }
